...
sub recursive_update {
    ...
    # _update_relation から呼ばれる際、一度に取得された rows が $related_rows にセットされます。
    # クエリの削減のために使用されます。
    my $related_rows;
    if ( exists $params{related_rows} ) {
        if ( $IN_RU ) {
            $related_rows = $params{related_rows};
        }
        else {
            die "parameter related_rows is private.";
        }
    }
    ...
    # object の select の為の条件を %pk_kvs に集めます。
    # $updates と belongs_to リレーションから構成されます。
    my %pk_kvs;
    for my $colname (@pks) {
        if (exists $updates->{$colname} && defined $updates->{$colname} && !ref $updates->{$colname}) {
    ...
    my @non_pk_columns = grep {
        ...
        sort keys %$updates;
    if (keys %pk_kvs != @pks && @non_pk_columns) {
        for my $key (keys %$updates) {
            ...
            my $val = do {
                if ( ref $updates->{$key} ) {
                    ...
                    die "Scalar cannot be specified for composite key table." unless @pk == 1;
                    +{ $pk[0] => $updates->{$key} };
                }
            };
            ...
            for my $colname (keys %$rel_cond) {
                next unless defined $rel_cond->{$colname} && any { $_ eq $colname } @pks;
                $pk_kvs{$colname} = $rel_cond->{$colname};
                $updates->{$colname} = delete $updates->{$key} if keys %$rel_cond == 1 && !ref $updates->{$key};
            }
        }
    }
...
$_\n";
    ...
    if ( !$object and $related_rows and keys %pk_kvs ) {
    ...
    if ( !$object and $related_rows and keys %pk_kvs ) {
        my $relink = any {
            ...
            $resolved->{$_} ne $pk_kvs{$_}
        } keys %$resolved;
        DEBUG and warn "check relink\n";
        unless ($relink) {
            DEBUG and warn "create new row for not relink\n";
            $object = $self->new_result({});
        }
    }

    # check if row can be found in resultset cache
    if ( !defined $object && !$related_rows && scalar keys %pk_kvs == scalar @pks ) {
